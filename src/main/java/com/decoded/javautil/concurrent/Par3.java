...
public class Par3<X, Y, Z> extends BasePar {
  private volatile X xResult = null;
  private volatile Y yResult = null;
  private volatile Z zResult = null;

  public Par3(CompletableFuture<X> futureX, CompletableFuture<Y> futureY, CompletableFuture<Z> futureZ) {
    super(3);
  ...
  public <R> CompletableFuture<R> map(Function3<X, Y, Z, R> resultMapper) {
    return getTask().thenApply(aVoid -> resultMapper.apply(xResult, yResult, zResult));
  }
   ...
   */
  public <R> CompletableFuture<R> flatMap(Function3<X, Y, Z, CompletableFuture<R>> resultMapper) {
    return getTask().thenCompose(aVoid -> resultMapper.apply(xResult, yResult, zResult));
  }
}
